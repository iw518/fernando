...
from app.main.GHole import *
from app.main.GLayer import *
from app.main.GPoint import *
from app.main.dataset import *
...
def find_holes_with_BG(projectNo):
    sql_str = ("SELECT soilhole.soil_holeNo, \
                ...
                SWITCH(grain.k025_0074>50, '粉砂', \
                       grain.k_0005<10, '砂质粉土', \
                       grain.k_0005<15, '粘质粉土', \
                       True, '粉质粘土' \
                      ) AS soilType \
...
def find_liqueholes(projectNo):
    holeDict = find_holes_with_BG(projectNo)
...
def ExportLayers_Stat(projectNo, mode=1):
    ...
    for item in keytuple:
        sql_str = sql_str+", Sum(SWITCH(titemdata.itemCode='"+item[0]+"',val(titemdata.iavg),True,0)) AS " +item[0]
