...
def PrintPdf(projectNo, probeInf, hole_list, index=None):
    import os
    from config import ROOTDIR
    cptPath = os.path.join(ROOTDIR, 'app', 'static', 'download')
    ...
    for i in range(len(hole_list)):
        if index is not None:
            i = index
            filename = projectNo + '__' + hole_list[i].holeName + '.pdf'
            # Attenion:where elments.extend must be used,but not elements.append
            elements.extend(Cpt2Pdf(hole_list[i], probeInf))
            break;
        else:
...
def Cpt2Pdf(hole, probeInf):
    pointsList = hole.testPoints
    L = len(pointsList)
    if L % 250 == 0:
        pageNums = L // 250
    else:
    ...
    for i in range(pageNums):
        ...
        elements.append(TemplateofCPTPDF(data, hole, probeInf))
        elements.append(PageBreak())
    return elements


# 静力触探记录表表格样式，其中的data(静力触探数据)通过函数CPT2PDF(xHole,probeInf)出入
def TemplateofCPTPDF(data, hole, probeInf):
    projectNo = hole.projectNo
    holeName = hole.holeName
    holeDep = (len(hole.testPoints)) / 10
