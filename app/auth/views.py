...
from ..main.core import import_motto
...
def register():
    form=RegisterForm()
    if form.validate_on_submit():
        user=User(phone=form.phone.data, email=form.email.data,nickname=form.nickname.data,fullname=form.fullname.data,password=form.password.data)
        db.session.add(user)
