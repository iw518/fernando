...
from flask import render_template, request, jsonify, Blueprint, json
...
def water():
    ...
    return render_template('calculation/water.html',
                           projectNo=projectNo,
                           holes=holes,
                           cnt=cnt,
                           rank=rank,
...
def natural_foundation():
    ...
    if request.method == "GET":
        ...
        return render_template('calculation/naturalfoundation.html',
                               projectNo=projectNo,
                               layers=layers,
                               manager=FindManager(projectNo)
                               )
...
@calculation.route('/pile', methods=['POST', 'GET'])
def pile():
    print(request.args)
    projectNo = request.args.get('projectNo')
            ...
            for layer in hole.layers:
                if not (layer.layerNo in repeat_layerNolist):
                    if layer.startDep < 6 and layer.endDep > 6:
                        ...
                        break
        return render_template('calculation/pile.html',
                           projectNo=projectNo,
                           holes=[xHole.holeName for xHole in hole_list],
                           manager=FindManager(projectNo),
                           layers=layers,
...
def liquefaction():
    ...
    return render_template('calculation/liquefaction.html',
                           projectNo=projectNo,
